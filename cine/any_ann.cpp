...
namespace cine2 {
  ...
  namespace ann_visitors {
    ...
    struct mutate
    {
      ...
      void operator()(T* state, size_t, size_t) const
      {
        ...
        if constexpr (Neuron::feedback_scratch) {
  ...
  #   pragma omp parallel for schedule(static,128)
      for (int p = 0; p < N; ++p) {
        if (pop[p].alive()) {
          ...
          for (int i = 0; i < L*L; ++i) {
            ...
            float eval = pann[p](input)[0] + noise(rnd::reng);    // ask ANN
    ...
    void mutate(const Param::ind_param& iparam, int N, bool fixed) override
    {
      ANN* __restrict pann = reinterpret_cast<ANN*>(state_);
