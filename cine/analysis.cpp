...
namespace cine2 {
  ...
  Analysis::Summary Analysis::assess_summary( const Population & Pop) const
  {
    ...
    for (size_t i = 0; i < Pop.pop.size(); ++i) {
      sfit += Pop.fitness[i];
      if (Pop.fitness[i] > 0.f) ++cfit;
    }
    std::set<int> unique_anc;
    for (const auto& ind : Pop.pop) {
      unique_anc.insert(ind.ancestor);
    }
    any_ann* tmp_ann = Pop.tmp_ann.get();
    double complexity = 0.0;
    std::set<const float*, ann_cmp> unique_ann(ann_cmp(tmp_ann->state_size()));
    ...
    return { 
      static_cast<int>(Pop.pop.size()),
      static_cast<float>(sfit / Pop.pop.size()), 
      cfit, 
