...
namespace cine2 {
    ...
    param_(param)
  {
    ...
    prey_.pop = std::vector<Individual>(param.prey.Ninit);
    prey_.tmp_pop = std::vector<Individual>(param.prey.Ninit);
    prey_.ann = make_any_ann(param.prey.L, param.Nmax, param.prey.ann.c_str());
    prey_.fitness = std::vector<float>(param.prey.Ninit, 0.f);
    prey_.tmp_ann = make_any_ann(param.prey.L, param.Nmax, param.prey.ann.c_str());

    pred_.pop = std::vector<Individual>(param.pred.Ninit);
    pred_.tmp_pop = std::vector<Individual>(param.pred.Ninit);
    pred_.ann = make_any_ann(param.pred.L, param.Nmax, param.pred.ann.c_str());
    pred_.fitness = std::vector<float>(param.pred.Ninit, 0.f);
    pred_.tmp_ann = make_any_ann(param.pred.L, param.Nmax, param.pred.ann.c_str());


    prey_.ann->initialize(param.prey);
    pred_.ann->initialize(param.pred);
  ...
  //  if (cm.un != Pop.pop.size()) throw cmd::parse_error("Number of ANNs doesn't match");
...
#     pragma omp parallel for schedule(static)
                               ...
                               bool fixed)
    {
      ...
      const auto& ann = *population.ann;
      const int N = static_cast<int>(pop.size());
      population.tmp_pop.clear();
      std::mutex mutex;
      // Beverton-Hold
      const auto G = iparam.alpha / (1.0 + iparam.beta * N);
#     pragma omp parallel 
      {
...
#       pragma omp for schedule(static)
        for (int i = 0; i < N; ++i) {
          if (pop[i].food + iparam.gamma > 0.f) {
            const auto nof = std::poisson_distribution<int>(G * (pop[i].food + iparam.gamma))(rnd::reng);
            for (int j = 0; j < nof; ++j) {
              auto newPos = pop[i].pos + Coordinate{ coorDist(rnd::reng), coorDist(rnd::reng) };
              mutex.lock();
              auto k = static_cast<int>(tmp_pop.size());
              if (k < Nmax - 1) {
                auto& ofs = tmp_pop.emplace_back();
                mutex.unlock();
                ofs.sprout(landscape.wrap(newPos), i);
                tmp_ann.assign(ann, i, k);   // copy ann
              }
              else {
                std::cerr << "Population exploded\n";
                mutex.unlock();
                std::abort();
              }
            }
      ...
      population.tmp_ann->mutate(iparam, static_cast<int>(population.tmp_pop.size()), fixed);
  ...
  bool Simulation::run(Observer* observer)
  {
    ...
    for (int gb = 0; gb < Gb; ++gb) {
      ...
      for (int tb = 0; tb < Tb; ++tb) {
	    ...
	    // clear fitness
	    //prey_.fitness.assign(prey_.pop.size(), 0.f);
      //pred_.fitness.assign(pred_.pop.size(), 0.f); 
      //prey_.rdist.mutate(prey_.fitness.cbegin(), prey_.fitness.cbegin() + prey_.pop.size());
      //pred_.rdist.mutate(pred_.fitness.cbegin(), pred_.fitness.cbegin() + pred_.pop.size());
      create_new_generations();
    }
    const int G = param_.G;
    for (g_ = 0; (g_ < G) && !(prey_.pop.empty() || pred_.pop.empty());  ++g_) {
      simulation_observer_notify(NEW_GENERATION);
...
#     pragma omp section
  ...
  {
    detail::create_new_generation(landscape_, prey_, param_.Nmax, param_.prey, fixed());
    detail::create_new_generation(landscape_, pred_, param_.Nmax, param_.pred, fixed());
  }
  ...
  void Simulation::resolve_grazing_and_attacks()
  {
    ...
    //*&*        grass(pos) = 0.f;  //   depleted
        ...
        if (grass(pos) < 0.f) {
          grass(pos) = 0.f; //*&*, necessary or resolved elsewhere?
        }
      }
    }
  ...
  public:
    ...
    bool notify(void* userdata, long long msg) override
    {
      ...
      case msg_type::GENERATION: {
        ...
        std::cout << watch_.elapsed<std::chrono::milliseconds>().count() << "ms\n";
        break;
      }
