...
#define clp_optional_vec(x, val) (clp.optional_vec( #x, val))
  ...
  Param parse_parameter(cmd::cmd_line_parser& clp)
  {
    ...
    clp_required(prey.Ninit);
    if (param.prey.Ninit > param.Nmax) {
      throw cmd::parse_error("prey.Ninit exceeds Nmax");
    }
    clp_optional_val(prey.L, 3);
    clp_required(prey.ann);

    clp_optional_val(prey.sprout_radius, 10000);
    clp_required(prey.alpha);
    clp_required(prey.beta);
    clp_required(prey.gamma);
    ...
    clp_required(pred.Ninit);
    if (param.pred.Ninit > param.Nmax) {
      throw cmd::parse_error("pred.Ninit exceeds Nmax");
    }
    clp_optional_val(pred.L, 3);
    clp_required(pred.ann);

    clp_optional_val(pred.sprout_radius, 10000);
    clp_required(pred.alpha);
    clp_required(pred.beta);
    clp_required(pred.gamma);
