...
input <- function() {
  ...
  colnames(prey) <- cn
  colnames(pred) <- cn
  list(prey=prey, pred=pred)
}
  
# load summary
summary <- function() {
  prey = matrix(import.raw(paste0(config$dir, '/prey_summary.bin'), numeric(), 8), ncol=6, byrow=T)
  pred = matrix(import.raw(paste0(config$dir, '/pred_summary.bin'), numeric(), 8), ncol=6, byrow=T)
  cn <- c('pop size', 'pop fitness', 'repro fitness', 'repro ind', 'repro clades', 'complexity')
  colnames(prey) <- cn
  colnames(pred) <- cn
  list(prey=prey, pred=pred)
}
  ...
  private:
    ...
    void stream_summary(std::ostream& os, const int N, const std::vector<Analysis::Summary>& summary)
    {
      ...
      for (size_t i = 0; i < g; ++i) {
        double val = summary[i].N; os.write((const char*)&val, sizeof(double));
        val = summary[i].ave_fitness; os.write((const char*)&val, sizeof(double));
    ...
    {
      {
        ...
        stream_summary(os, static_cast<int>(sim->prey().pop.size()), sim->analysis().prey_summary());
      }
      {
        std::ofstream os;
        ...
        stream_summary(os, static_cast<int>(sim->pred().pop.size()), sim->analysis().pred_summary());
      }
      {
        std::ofstream os;
