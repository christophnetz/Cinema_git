...
namespace cinema {
    ...
    sim_(sim)
  {
    ...
    GLsizei size = static_cast<GLsizei>(prey_ann_.type_size) * prey_ann_.Nmax;
    ...
    size = static_cast<GLsizei>(pred_ann_.type_size) * pred_ann_.Nmax;
  ...
  void GLSimState::flush_async(const cine2::Simulation& sim, long long msg)
  {
    ...
    case msg_type::NEW_GENERATION:
      prey_ann_.N0 = static_cast<int>(sim.prey().pop.size());
      pred_ann_.N0 = static_cast<int>(sim.pred().pop.size());
      std::memcpy(ptr_[VBO::VBO_PREY_ANN], sim.prey().ann->data(), sim.prey().pop.size() * prey_ann_.type_size);
      std::memcpy(ptr_[VBO::VBO_PRED_ANN], sim.pred().ann->data(), sim.pred().pop.size() * pred_ann_.type_size);
    case msg_type::POST_TIMESTEP: {
      std::memcpy(ptr_[VBO::VBO_LAYER], sim.landscape().data(), 3ull * dim_* dim_ * sizeof(float));
      break;
    }
    }
  }
