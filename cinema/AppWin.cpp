...
namespace cinema { 
  ...
  LRESULT AppWin::OnNotify(UINT, WPARAM wParam, LPARAM lParam, BOOL&)
  {
    ...
    case msg_type::NEW_GENERATION:
        ...
        snprintf(buf, 128, "Prey %zd", simulation()->prey().pop.size());
        StatusBar_.SetPaneText(ID_PANE_PREY, CA2T(buf));
        snprintf(buf, 128, "Pred %zd", simulation()->pred().pop.size());
        StatusBar_.SetPaneText(ID_PANE_PRED, CA2T(buf));
    case msg_type::POST_TIMESTEP:
  ...
  LRESULT AppWin::OnCreate(UINT, WPARAM, LPARAM, BOOL&)
  {
    ...
    int arrPanes[] = { ID_DEFAULT_PANE, ID_PANE_GENERATION, ID_PANE_PREY, ID_PANE_PRED, ID_PANE_TIMESTEP, ID_PANE_MS };
