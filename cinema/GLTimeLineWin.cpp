...
namespace cinema {
  ...
  void GLTimeLineWin::flush_async(long long msg)
  {
    ...
    switch (msg) {
      case Msg::GENERATION:
        ...
        p[1] = y = (s.ave_fitness * sim_state_->sim()->pred().pop.size()) / s.repro_ind; max_[1] = std::max(max_[1], y);
        ...
        p[6] = y = (s.ave_fitness * sim_state_->sim()->prey().pop.size()) / s.repro_ind; max_[6] = std::max(max_[6], y);
  ...
  void GLTimeLineWin::on_create()
  {
    ...
    //ttc_.Create(m_hWnd, rcDefault, NULL, WS_POPUP | TTS_NOPREFIX | TTS_NOFADE | TTS_NOANIMATE, 0);
    //ttc_.SetDelayTime(TTDT_INITIAL, 0);
    //ttc_.SetDelayTime(TTDT_RESHOW, 0);
    //auto toolInfo = CToolInfo(TTF_IDISHWND | TTF_TRACK | TTF_ABSOLUTE, ttc_, 0, NULL, L"WWW...");
    //ttc_.AddTool(&toolInfo);
  }
  ...
  void GLTimeLineWin::on_mouse_hover(int winx, int winy)
  {
    int dummy = 0;
  }


  void GLTimeLineWin::on_mouse_move(int winx, int winy)
  {
    // enable hover messages
    auto tt = TRACKMOUSEEVENT{
      DWORD(sizeof(TRACKMOUSEEVENT)),
      TME_HOVER,
      this->m_hWnd,
      300
    };
