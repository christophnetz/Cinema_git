...
namespace cinema {
  ...
  struct AnnRenderer
  {
    ...
    {
      auto world = glm::dvec4(0, ann.weights, 0, ann.N0);
    ...
    void init_glsl()
    {
      ...
      glTextureStorage2D(tex, 1, GL_R32F, ann.weights, ann.Nmax);
    ...
    void render_hist(GLuint prog, GLuint rProg, const float* pw)
    {
      ...
      for (int i = 0; i < ann.N0; ++i, pw += ann.stride) {
    ...
    void render(GLuint prog, GLuint rProg, GLuint vbo, GLint pack, const float* ptr)
    {
      ...
      auto mm = std::minmax_element(ptr, ptr + ann.stride * ann.N0);
      glBindBuffer(GL_PIXEL_UNPACK_BUFFER, vbo);
      glPixelStorei(GL_UNPACK_ROW_LENGTH, pack);
      glTextureSubImage2D(tex, 0, 0, 0, ann.weights, ann.N0, GL_RED, GL_FLOAT, nullptr);
      glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
      glBindBuffer(GL_PIXEL_UNPACK_BUFFER, GL_NONE);
